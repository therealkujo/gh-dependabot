...
class TestDependabot(unittest.TestCase):
    ...
    mock_gh_query = ['/opt/homebrew/bin/gh', 'api', 'graphql', '-F', 'org=github', '-F', 'repo=foo', '-F', 'cursor=null', '-f', 'query=query ($org: String! $repo: String! $cursor: String){       repository(owner: $org name: $repo) {         name         vulnerabilityAlerts(first: 100 after: $cursor) {           pageInfo {             hasNextPage             endCursor           }           totalCount           nodes {             id             securityAdvisory {               ...advFields             }             securityVulnerability {               package {                 ...pkgFields               }               vulnerableVersionRange             }             createdAt             state             fixedAt             fixReason             dismissedAt             dismissReason             dismisser {                 login             }             vulnerableManifestPath             vulnerableRequirements           }         }       }     }     fragment advFields on SecurityAdvisory {       ghsaId       permalink       severity       description       summary     }     fragment pkgFields on SecurityAdvisoryPackage {       name       ecosystem     }     ']
    mock_gh_paginated_query = ['/opt/homebrew/bin/gh', 'api', 'graphql', '-F', 'org=github', '-F', 'repo=foo', '-F', 'cursor=Y3Vyc29yOnYyOpHOr2XWzA==', '-f', 'query=query ($org: String! $repo: String! $cursor: String){       repository(owner: $org name: $repo) {         name         vulnerabilityAlerts(first: 100 after: $cursor) {           pageInfo {             hasNextPage             endCursor           }           totalCount           nodes {             id             securityAdvisory {               ...advFields             }             securityVulnerability {               package {                 ...pkgFields               }               vulnerableVersionRange             }             createdAt             state             fixedAt             fixReason             dismissedAt             dismissReason             dismisser {                 login             }             vulnerableManifestPath             vulnerableRequirements           }         }       }     }     fragment advFields on SecurityAdvisory {       ghsaId       permalink       severity       description       summary     }     fragment pkgFields on SecurityAdvisoryPackage {       name       ecosystem     }     ']
